...
- `from-template-report.docx` — fresh body built on the cloned styles of `03-custom_styles.docx` (Q3 report with `Titolo`/`Sottotitolo`/`Titolo1`/`Titolo2` paragraphs and a metrics table).
- `open-template-appendix.docx` — `04-custom_styles_plus_sample_text.docx`'s original paragraphs preserved, plus an appended `Titolo1`-style heading, a paragraph, and a 4-column builder table.
...
- `doc1.AddParagraph("Q3 Financial Report").SetStyle("Titolo")` — uses the **Italian** style IDs straight from the cloned `styles.xml`. Because those IDs are defined in the template, no warnings are emitted (unlike Examples 02/08 which use English IDs against the default style table).
